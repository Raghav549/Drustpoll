...
export async function getAccountExperiencePreferences(userId:string):Promise<ExperiencePrefs>{
 ...
 if(r.rows[0])return r.rows[0];await query('INSERT INTO account_experience_preferences(user_id) VALUES($1) ON CONFLICT DO NOTHING',[userId]);
...
export async function updateAccountExperiencePreferences(userId:string,input:Record<string,unknown>):Promise<ExperiencePrefs>{
 const current=await getAccountExperiencePreferences(userId);const appearance=clean(input.appearance??current.appearance,20);if(!allowedAppearance.has(appearance))throw new Error('Invalid appearance');
 const language=clean(input.language??current.language,16)||'en';const region=clean(input.region??current.region,16)||'IN';const currency=clean(input.currency??current.currency,3).toUpperCase()||'INR';
 const bool=(key:BoolKey)=>{const value=input[key];return value===undefined?current[key]:value===true;};
 const vals={appearance,language,region,currency,reduced_motion:bool('reduced_motion'),data_saver:bool('data_saver'),large_text:bool('large_text')};
 const r=await query<ExperiencePrefs>(`INSERT INTO account_experience_preferences(user_id,appearance,language,region,currency,reduced_motion,data_saver,large_text) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(user_id) DO UPDATE SET appearance=EXCLUDED.appearance,language=EXCLUDED.language,region=EXCLUDED.region,currency=EXCLUDED.currency,reduced_motion=EXCLUDED.reduced_motion,data_saver=EXCLUDED.data_saver,large_text=EXCLUDED.large_text,updated_at=now() RETURNING appearance,language,region,currency,reduced_motion,data_saver,large_text`,[userId,appearance,language,region,currency,vals.reduced_motion,vals.data_saver,vals.large_text]);
 await query(`INSERT INTO privacy_control_history(user_id,control,value) VALUES($1,'experience',$2)`,[userId,JSON.stringify(vals)]);return r.rows[0];
}
