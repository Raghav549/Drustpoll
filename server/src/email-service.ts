...
const appUrl=(process.env.PUBLIC_ORIGIN??'https://drustpoll.app').replace(/\/$/,'');

function transport(){
 if(!host||!user||!pass)throw new Error('Email delivery is not configured. Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS and SMTP_FROM.');
 return nodemailer.createTransport({host,port,secure:port===465,auth:{user,pass},tls:{minVersion:'TLSv1.2'}});
}
