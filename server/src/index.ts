...
import { createRepost, getCommentReplies, getFeedPreferences, getPoll, getPostComments, listFeedTopics, listHiddenTopics, recordRecommendationFeedback, removeRecommendationFeedback, resetFeed, setFeedPreferences, setHiddenTopic, votePoll } from './social-surface-service.js';
...
const server=createServer(async(req,res)=>{try{
 ...
 if(req.method==='GET'&&url.pathname==='/v1/discovery/results')return json(res,200,await getDiscoveryResults(session.userId,url.searchParams));
 if(req.method==='GET'&&url.pathname==='/v1/discovery/feed')return json(res,200,await getDiscoveryFeed(session.userId,url.searchParams));
 if(req.method==='GET'&&url.pathname==='/v1/discovery/recent-searches')return json(res,200,await listRecentSearches(session.userId,Number(url.searchParams.get('limit')??20)));
 if(req.method==='DELETE'&&url.pathname==='/v1/discovery/recent-searches')return json(res,200,await clearRecentSearches(session.userId));
 if(req.method==='GET'&&url.pathname==='/v1/discovery/saved-searches')return json(res,200,await listSavedSearches(session.userId));
 if(req.method==='POST'&&url.pathname==='/v1/discovery/saved-searches'){const i=await body(req);return json(res,201,await saveSearch(session.userId,String(i.query??''),i.kind??'all'));}
 const ss=url.pathname.match(/^\/v1\/discovery\/saved-searches\/([^/]+)$/);if(req.method==='DELETE'&&ss)return json(res,200,await deleteSavedSearch(session.userId,ss[1]));
 if(req.method==='GET'&&url.pathname==='/v1/discovery/categories')return json(res,200,{categories:await listDiscoveryCategories(session.userId)});
 if(req.method==='GET'&&url.pathname==='/v1/discovery/preferences')return json(res,200,{preferences:await getDiscoveryPreferences(session.userId)});
 if(req.method==='PUT'&&url.pathname==='/v1/discovery/preferences')return json(res,200,{preferences:await updateDiscoveryPreferences(session.userId,await body(req))});
 ...
 if(req.method==='GET'&&url.pathname==='/v1/reels/preferences')return json(res,200,{preferences:await getReelPreferences(session.userId)});
 if(req.method==='PUT'&&url.pathname==='/v1/reels/preferences')return json(res,200,{preferences:await updateReelPreferences(session.userId,await body(req))});
 if(req.method==='GET'&&url.pathname==='/v1/reels/audio')return json(res,200,{audio:await getReelAudio(url.searchParams.get('q')??'',Number(url.searchParams.get('limit')??20))});
 if(req.method==='GET'&&url.pathname==='/v1/reels/related')return json(res,200,{items:await getRelatedReels(session.userId,String(url.searchParams.get('postId')??''),Number(url.searchParams.get('limit')??12))});
 const rcf=url.pathname.match(/^\/v1\/reels\/creators\/([^/]+)\/feedback$/);if(req.method==='POST'&&rcf){const i=await body(req);return json(res,200,await setReelCreatorFeedback(session.userId,rcf[1],String(i.signal??'')));}
 ...
 const pv=url.pathname.match(/^\/v1\/posts\/([^/]+)\/comments$/);if(req.method==='GET'&&pv)return json(res,200,await getPostComments(session.userId,pv[1],Number(url.searchParams.get('limit')??50),url.searchParams.get('before')??undefined));
 const cr=url.pathname.match(/^\/v1\/comments\/([^/]+)\/replies$/);if(req.method==='GET'&&cr)return json(res,200,await getCommentReplies(session.userId,cr[1],Number(url.searchParams.get('limit')??50),url.searchParams.get('before')??undefined));
 ...
 if(req.method==='POST'&&url.pathname==='/v1/feed/reset')return json(res,200,await resetFeed(session.userId));
 if(req.method==='GET'&&url.pathname==='/v1/feed/hidden-topics')return json(res,200,{topics:await listHiddenTopics(session.userId)});
 if(req.method==='POST'&&url.pathname==='/v1/feed/hidden-topics'){const i=await body(req);return json(res,200,await setHiddenTopic(session.userId,String(i.topic??''),true));}
 if(req.method==='DELETE'&&url.pathname==='/v1/feed/hidden-topics'){const i=await body(req);return json(res,200,await setHiddenTopic(session.userId,String(i.topic??''),false));}
 const sp=url.pathname.match(/^\/v1\/profiles\/([^/]+)$/);if(req.method==='GET'&&sp)return json(res,200,{profile:await getProfileSurface(session.userId,sp[1])});
 if(req.method==='GET'&&url.pathname==='/v1/profiles/me')return json(res,200,{profile:await getProfileSurface(session.userId,session.userId)});
 ...
 const seller=url.pathname.match(/^\/v1\/profiles\/([^/]+)\/seller$/);if(req.method==='GET'&&seller)return json(res,200,{seller:await getSellerSummary(session.userId,seller[1])});
 if(req.method==='PUT'&&url.pathname==='/v1/profiles/me/extras')return json(res,200,await updateProfileExtras(session.userId,await body(req)));
 ...
 const prod=url.pathname.match(/^\/v1\/shop\/products\/([^/]+)$/);if(req.method==='GET'&&prod)return json(res,200,{product:await getProduct(session.userId,prod[1])});
 if(req.method==='POST'&&url.pathname==='/v1/shop/events'){return json(res,202,await recordCommerceEvent(session.userId,await body(req)));}
 if(req.method==='GET'&&url.pathname==='/v1/cart')return json(res,200,await getCart(session.userId));
 if(req.method==='POST'&&url.pathname==='/v1/cart/items'){const i=await body(req);return json(res,200,await addCartItem(session.userId,String(i.productId??''),Number(i.quantity??0)));}
 ...
 if(req.method==='POST'&&url.pathname==='/v1/orders')return json(res,201,await placeOrder(session.userId,String((await body(req)).idempotencyKey??'')));
