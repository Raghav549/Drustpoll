...
const server=createServer(async(req,res)=>{try{
 ...
 if(req.method==='POST'&&url.pathname==='/v1/recommendation/feedback'){const i=await body(req);return json(res,202,await recordRecommendationFeedback(session.userId,i.objectType,String(i.objectId??''),i.signal,String(i.context??'feed')));}
 if(req.method==='DELETE'&&url.pathname==='/v1/recommendation/feedback'){const i=await body(req);return json(res,200,await removeRecommendationFeedback(session.userId,i.objectType,String(i.objectId??''),i.signal,String(i.context??'feed')));}
 ...
 if(req.method==='GET'&&url.pathname==='/v1/feed'){const m=(url.searchParams.get('mode')??'for_you');if(!['for_you','following','latest'].includes(m))return json(res,400,{error:'Invalid feed mode'});if(m==='latest')return json(res,200,await getFeed(session.userId,'for_you',Number(url.searchParams.get('limit')??30),url.searchParams.get('before')??undefined));return json(res,200,await getFeed(session.userId,m as 'for_you'|'following',Number(url.searchParams.get('limit')??30),url.searchParams.get('before')??undefined));}
 ...
 if(req.method==='GET'&&url.pathname==='/v1/feed/hidden-topics')return json(res,200,await listHiddenTopics(session.userId));
 ...
 const cm=url.pathname.match(/^\/v1\/posts\/([^/]+)\/comments$/);if(cm){if(req.method==='GET')return json(res,200,await getPostComments(session.userId,cm[1],Number(url.searchParams.get('limit')??50),url.searchParams.get('before')??undefined));if(req.method==='POST'){const i=await body(req);return json(res,201,await addComment(session.userId,cm[1],String(i.body??''),i.parentId?String(i.parentId):undefined));}}
 const rr=url.pathname.match(/^\/v1\/comments\/([^/]+)\/replies$/);if(req.method==='GET'&&rr)return json(res,200,await getCommentReplies(session.userId,rr[1],Number(url.searchParams.get('limit')??50),url.searchParams.get('before')??undefined));
 const pp=url.pathname.match(/^\/v1\/posts\/([^/]+)\/poll$/);if(req.method==='GET'&&pp)return json(res,200,{poll:await getPoll(session.userId,pp[1])});
 const pv=url.pathname.match(/^\/v1\/posts\/([^/]+)\/poll\/vote$/);if(req.method==='POST'&&pv){const i=await body(req);return json(res,200,await votePoll(session.userId,pv[1],String(i.optionId??'')));}
 ...
 }catch(error){console.error(error);return json(res,400,{error:error instanceof Error?error.message:'Request failed'});}});
