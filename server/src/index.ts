...
import { addCartItem, createProduct, getCart, getProduct, listSellerProducts, placeOrder } from './commerce-service.js';
...
const server=createServer(async(req,res)=>{try{
  ...
  if(req.method==='POST'&&url.pathname==='/v1/auth/otp/request'){const i=await body(req),p=String(i.purpose??''),d=String(i.destination??'');if(!d||!['verify_email','verify_phone','login_step_up','password_reset'].includes(p))return json(res,400,{error:'Invalid OTP request'});const s=await auth(req);if(!s&&!['verify_email','verify_phone','password_reset'].includes(p))return json(res,401,{error:'Unauthenticated'});return json(res,202,await requestOtp(s?.userId??null,d,p as any,ip));}
...
}catch(error){const message=error instanceof Error?error.message:'Request failed';const status=/Unauthenticated/i.test(message)?401:/too many/i.test(message)?429:/already in use|invalid|incorrect|expired|required|not found|Cannot|insufficient|inactive|blocked|moderation|MEDIA_STORAGE_NOT_CONFIGURED|MULTIMODAL_EXTRACTOR_NOT_CONFIGURED/i.test(message)?400:500;return json(res,status,{error:message});}});
