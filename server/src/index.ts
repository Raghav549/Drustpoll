...
const server=createServer(async(req,res)=>{try{
 ...
 if(req.method==='POST'&&url.pathname==='/v1/auth/otp/request'){const i=await body(req);const p=String(i.purpose??''),d=String(i.destination??'');if(!d||!['verify_email','verify_phone','login_step_up','password_reset'].includes(p))return json(res,400,{error:'Invalid OTP request'});const s=await auth(req);if(!s&&!['verify_email','verify_phone','password_reset'].includes(p))return json(res,401,{error:'Unauthenticated'});return json(res,202,await requestOtp(s?.userId??null,d,p as any,ip));}
 ...
 const rm=url.pathname.match(/^\/v1\/auth\/sessions\/([^/]+)$/);if(req.method==='DELETE'&&rm){await revokeSession(session.userId,rm[1]);return json(res,200,{ok:true});}
 if(req.method==='POST'&&url.pathname==='/v1/auth/logout-all'){await revokeAll(session.userId);clearCookie(res);return json(res,200,{ok:true});}
 ...
 if(req.method==='POST'&&url.pathname==='/v1/auth/password/change'){const i=await body(req);await changePassword(session.userId,String(i.currentPassword??''),String(i.newPassword??''));clearCookie(res);return json(res,200,{ok:true,sessionsRevoked:true});}
 ...
 if(req.method==='POST'&&url.pathname==='/v1/safety/block'){const i=await body(req);return json(res,200,await blockUser(session.userId,String(i.targetUserId??'')));}if(req.method==='DELETE'&&url.pathname==='/v1/safety/block'){const i=await body(req);return json(res,200,await unblockUser(session.userId,String(i.targetUserId??'')));}if(req.method==='POST'&&url.pathname==='/v1/safety/mute'){const i=await body(req);return json(res,200,await muteUser(session.userId,String(i.targetUserId??'')));}if(req.method==='DELETE'&&url.pathname==='/v1/safety/mute'){const i=await body(req);return json(res,200,await unmuteUser(session.userId,String(i.targetUserId??'')));}if(req.method==='GET'&&url.pathname==='/v1/safety/state'){const id=url.searchParams.get('targetUserId');if(!id)return json(res,400,{error:'targetUserId required'});return json(res,200,await getSafetyState(session.userId,id));}if(req.method==='POST'&&url.pathname==='/v1/safety/report'){const i=await body(req);return json(res,201,await reportContent(session.userId,i.target??{},String(i.reason??''),String(i.details??'')));}
 ...
 if(req.method==='POST'&&url.pathname==='/v1/social/follow'){const i=await body(req);return json(res,200,await follow(session.userId,String(i.targetUserId??'')));}if(req.method==='DELETE'&&url.pathname==='/v1/social/follow'){const i=await body(req);return json(res,200,await unfollow(session.userId,String(i.targetUserId??'')));}if(req.method==='POST'&&url.pathname==='/v1/social/relationship'){const i=await body(req);return json(res,200,await setFollowState(session.userId,String(i.targetUserId??''),i.state));}
 if(req.method==='GET'&&url.pathname==='/v1/feed'){const m=url.searchParams.get('mode')==='following'?'following':'for_you';return json(res,200,await getFeed(session.userId,m,Number(url.searchParams.get('limit')??30),url.searchParams.get('before')??undefined));}if(req.method==='GET'&&url.pathname==='/v1/feed/recommended')return json(res,200,{items:await getRecommendedPosts(session.userId,Number(url.searchParams.get('limit')??20))});if(req.method==='POST'&&url.pathname==='/v1/feed/events'){const i=await body(req);return json(res,202,await recordFeedEvents(session.userId,Array.isArray(i.events)?i.events:[]));}
