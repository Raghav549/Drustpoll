...
export async function api<T>(path:string,init:RequestInit={},retry=true):Promise<T>{if(!API_URL)throw new ApiError(0,'EXPO_PUBLIC_API_URL is not configured');const token=await getAccessToken();const headers=new Headers(init.headers);headers.set('Content-Type','application/json');if(token)headers.set('Authorization',`Bearer ${token}`);const res=await fetchWithTimeout(`${API_URL}${path}`,{...init,credentials:'include',headers});const text=await res.text();let data:any={};try{data=text?JSON.parse(text):{};}catch{data={error:'Invalid server response'};}if(res.status===401&&retry&&token){refreshInFlight??=refreshSession().finally(()=>{refreshInFlight=null;});const refreshed=await refreshInFlight;if(refreshed?.token)return api<T>(path,init,false);}if(!res.ok)throw new ApiError(res.status,String(data.error??`Request failed (${res.status})`));return data as T;}
