...
export function AuthProvider({children}:PropsWithChildren){
 ...
 useEffect(()=>{if(state.status==='loading')return;const publicPath=PUBLIC_PATHS.has(pathname);if(state.status==='signed_out'&&!publicPath)router.replace('/auth');else if(state.status==='signed_in'&&pathname==='/auth')router.replace('/');},[state.status,pathname]);
