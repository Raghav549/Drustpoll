...
async function request(path:string,init:RequestInit={},withAuth=true){
 ...
 const token=withAuth?await loadToken():null;const headers=new Headers(init.headers);headers.set('Content-Type','application/json');if(token)headers.set('Authorization',`Bearer ${token}`);
