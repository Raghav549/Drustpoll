...
async function saveTokens(token:string,refreshToken?:string){memoryToken=token;memoryRefresh=refreshToken??null;const store=await secureStore();if(store){await store.setItemAsync('drustpoll.session',token);if(refreshToken)await store.setItemAsync('drustpoll.refresh',refreshToken);}}
...
export async function clearPersistedAuth(){memoryToken=null;memoryRefresh=null;const store=await secureStore();if(store){await store.deleteItemAsync('drustpoll.session');await store.deleteItemAsync('drustpoll.refresh');}}
...
export async function signUp(input:{username:string;displayName:string;password:string;email?:string;phone?:string}){const data=await request('/v1/auth/signup',{method:'POST',body:JSON.stringify(input)},false);await clearPersistedAuth();if(data.token)await saveTokens(data.token,data.refreshToken);return data;}
