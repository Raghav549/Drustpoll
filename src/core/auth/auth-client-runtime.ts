...
async function request(path:string,init:RequestInit={}){
 ...
 const token=await loadToken();const headers=new Headers(init.headers);headers.set('Content-Type','application/json');if(token)headers.set('Authorization',`Bearer ${token}`);
