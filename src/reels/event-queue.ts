...
export async function flushReelEvents(sessionId:string){const current=await read();if(!current.length)return 0;const batch=current.slice(0,100);try{const result=await recordReelWatchEvents(sessionId,batch);if(result.acceptedClientEventIds.length){const accepted=new Set(result.acceptedClientEventIds);await write(current.filter(x=>!accepted.has(x.clientEventId)));}return result.accepted;}catch{return 0;}}
